...
void batchMatching();
void writeToFile(const vector <Point>& Traj,string path);
//////////////////////////////////////////////
...
vector <Point> FindMatchedSequence(int i,int k,vector <MAT>& fi_i,vector <double>& Wi){
	...
	for(int t=0;t<(int)candiPoint[0].size();++t)
	...
	if(i == 0){//i = 0,set init value be -max because there is no fi[i][0],only have fi[i][1]
		...
		for(int s = 0;s<(int)candiPoint[i].size();++s){
			if(s == k) continue;
			int tSz = 0;
	...
	for(int j=1;j<pNum;++j){
		...
		for(int s = 0;s<(int)candiPoint[j].size();++s){
			for(int t=0;t<(int)candiPoint[j-1].size();++t){
	...
	for(int s=0;s<(int)candiPoint[pNum-1].size();++s){
